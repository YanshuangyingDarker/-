...
extern NSDate *dateFromString(NSString *string);

extern NSDateFormatter *standerdFormatter(void);
/**
 ...
 * Implementations of `NSCoding`, `NSCopying`, `-hash` and `-isEqual:`.
 ...
 See `XHModel` protocol for custom methods.
 
 
 
 int main() {
 // create model from json
 XHBook *book = [XHBook modelWithJSON:@"{\"name\": \"Harry Potter\", \"pages\": 256, \"author\": {\"name\": \"J.K.Rowling\", \"birthday\": \"1965-07-31\" }}"];
 ...
 */
@interface NSDictionary (XHModel)
...
/**
 ...
 There's no need to add '<XHModel>' to your class header.
 */
@protocol XHModel <NSObject>
@optional
...
- (void)modelWillStartTransFromDictionary:(NSMutableDictionary *)dic;

- (void)modelDidCompeleteTransFromDictionary:(NSMutableDictionary *)dic;
