...
//  XHKit
//
//  Created by Xaviar on 12/10/15.
//  Copyright © 2015 Xaviar. All rights reserved.
//

#import "UIImage+XHAdd.h"
#import "UIDeviceAddition.h"
#import "NSStringAddition.h"
#import "XHKitMacro.h"
#import "XHCGUtilities.h"
#import <ImageIO/ImageIO.h>
#import <Accelerate/Accelerate.h>
#import <CoreText/CoreText.h>
#import <objc/runtime.h>

XHSYNTH_DUMMY_CLASS(UIImage_XHAdd)

static NSTimeInterval _XH_CGImageSourceGetGIFFrameDelayAtIndex(CGImageSourceRef source, size_t index) {
...
+ (UIImage *)imageWithSmallGIFData:(NSData *)data scale:(CGFloat)scale {
    ...
    for (size_t i = 0; i < count; i++) {
        NSTimeInterval delay = _XH_CGImageSourceGetGIFFrameDelayAtIndex(source, i);
        totalTime += delay;
...
+ (UIImage *)imageWithPDF:(id)dataOrPath {
    return [self _XH_imageWithPDF:dataOrPath resize:NO size:CGSizeZero];
}

+ (UIImage *)imageWithPDF:(id)dataOrPath size:(CGSize)size {
    return [self _XH_imageWithPDF:dataOrPath resize:YES size:size];
}
...
+ (UIImage *)_XH_imageWithPDF:(id)dataOrPath resize:(BOOL)resize size:(CGSize)size {
...
- (void)drawInRect:(CGRect)rect withContentMode:(UIViewContentMode)contentMode clipsToBounds:(BOOL)clips{
    CGRect drawRect = XHCGRectFitWithContentMode(rect, self.size, contentMode);
...
- (UIImage *)imageByCropToCenterRect:(CGRect)rect {
    
    CGFloat width = self.size.width;
    CGFloat height = self.size.height;
    
    CGFloat topOffset = fabs((rect.size.height - height))/2;
    CGFloat leftOffset = fabs((rect.size.width - width))/2;
    
    return [self imageByInsetEdge:UIEdgeInsetsMake(topOffset, leftOffset, topOffset, leftOffset) withColor:nil];
}
...
- (UIImage *)_XH_flipHorizontal:(BOOL)horizontal vertical:(BOOL)vertical {
...
- (UIImage *)imageByRotate180 {
    return [self _XH_flipHorizontal:YES vertical:YES];
}

- (UIImage *)imageByFlipVertical {
    return [self _XH_flipHorizontal:NO vertical:YES];
}

- (UIImage *)imageByFlipHorizontal {
    return [self _XH_flipHorizontal:YES vertical:NO];
}
...
- (UIImage *)imageByBlurRadius:(CGFloat)blurRadius
    ...
    if (self.size.width < 1 || self.size.height < 1) {
        NSLog(@"UIImage+XHAdd error: invalid size: (%.2f x %.2f). Both dimensions must be >= 1: %@", self.size.width, self.size.height, self);
        return nil;
    }
    if (!self.CGImage) {
        NSLog(@"UIImage+XHAdd error: inputImage must be backed by a CGImage: %@", self);
        return nil;
    }
    if (maskImage && !maskImage.CGImage) {
        NSLog(@"UIImage+XHAdd error: effectMaskImage must be backed by a CGImage: %@", maskImage);
        return nil;
    }
    ...
    if (!hasBlur && !hasSaturation) {
        return [self _XH_mergeImageRef:imageRef tintColor:tintColor tintBlendMode:tintBlendMode maskImage:maskImage opaque:opaque];
    }
    ...
    if (hasNewFunc) {
        ...
        if (err != kvImageNoError) {
            NSLog(@"UIImage+XHAdd error: vImageBuffer_InitWithCGImage returned error code %zi for inputImage: %@", err, self);
            return nil;
        }
        err = vImageBuffer_Init(&scratch, effect.height, effect.width, format.bitsPerPixel, kvImageNoFlags);
        if (err != kvImageNoError) {
            NSLog(@"UIImage+XHAdd error: vImageBuffer_Init returned error code %zi for inputImage: %@", err, self);
            return nil;
        }
    } else {
    ...
    if (hasNewFunc) {
        ...
        effectCGImage = vImageCreateCGImageFromBuffer(input, &format, &_XH_cleanupBuffer, NULL, kvImageNoAllocate, NULL);
        if (effectCGImage == NULL) {
            effectCGImage = vImageCreateCGImageFromBuffer(input, &format, NULL, NULL, kvImageNoFlags, NULL);
            free(input->data);
        }
        free(output->data);
        outputImage = [self _XH_mergeImageRef:effectCGImage tintColor:tintColor tintBlendMode:tintBlendMode maskImage:maskImage opaque:opaque];
...
static void _XH_cleanupBuffer(void *userData, void *buf_data) {
    free(buf_data);
}

// Helper function to add tint and mask.
- (UIImage *)_XH_mergeImageRef:(CGImageRef)effectCGImage
                     tintColor:(UIColor *)tintColor
